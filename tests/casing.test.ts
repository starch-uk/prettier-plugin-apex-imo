...
describe('casing', () => {
	describe('normalizeStandardObjectType', () => {
		it('should return unchanged for non-standard types', () => {
			expect(normalizeStandardObjectType('MyCustomClass')).toBe(
				'MyCustomClass',
			);
		});
		...
		describe('all standard objects', () => {
			// Test all standard objects normalize correctly with mixed case input
			// Using .concurrent for parallel execution of independent tests
			test.concurrent.each(Object.entries(STANDARD_OBJECTS))(
				'should normalize mixed case %s to %s',
				(lowercaseKey, expectedPascalCase) => {
					// Test mixed case variation (alternating uppercase/lowercase)
					const alternateModulo = 2;
					const alternateStartIndex = 0;
					const mixedCase = lowercaseKey
						.split('')
						.map((char, i) =>
						...
						.join('');
					expect(normalizeStandardObjectType(mixedCase)).toBe(
						expectedPascalCase,
					);
					// Test idempotency - PascalCase should normalize to itself
					expect(
						normalizeStandardObjectType(expectedPascalCase),
					).toBe(expectedPascalCase);
				},
			);

			it('should have all values in proper PascalCase or camelCase format', () => {
				// Allow both PascalCase (starts with uppercase) and camelCase (starts with lowercase)
				// Also allow underscores (e.g., Knowledge__DataCategorySelection)
				const caseFormatRegex = /^[a-zA-Z][a-zA-Z0-9_]*$/;
				for (const [, value] of Object.entries(STANDARD_OBJECTS)) {
					// Skip special cases with spaces or parentheses
					if (value.includes(' ') || value.includes('(')) {
						continue;
					}
					expect(value).toMatch(caseFormatRegex);
				}
			});

			it('should have all keys in lowercase format', () => {
				for (const key of Object.keys(STANDARD_OBJECTS)) {
					expect(key).toBe(key.toLowerCase());
				}
			});

			it('should not have duplicate entries', () => {
				const values = Object.values(STANDARD_OBJECTS);
	...
	describe('isIdentifier', () => {
		it('should identify Identifier class nodes', () => {
			const node = {
		...
		it('should identify nodes with Identifier in class name', () => {
			const node = {
		...
		it('should identify nodes with value property', () => {
			const node = {
		...
		it('should reject nodes without value property', () => {
			const node = {
				[nodeClassKey]: 'apex.jorje.data.ast.SomeNode',
			} as ApexNode;
	...
	describe('isInTypeContext', () => {
		...
		it('should return false for "field" key when FromExpr is not in stack', () => {
			const path = createMockPath({} as ApexNode, 'field');
			expect(isInTypeContext(path)).toBe(false);
		});

		it('should return true when parent is TypeRef', () => {
			const parentNode = {
		...
		it('should return true when parent class includes "Type" but not "Variable"', () => {
			const parentNode = {
		...
		it('should return false when parent class includes "Type" and "Variable" with key "name"', () => {
			const parentNode = {
		...
		it('should return true when key is "name" and parent class includes "Type" but not "Variable"', () => {
			const parentNode = {
				[nodeClassKey]: 'apex.jorje.data.ast.SomeTypeNode',
			} as ApexNode;
			// Stack length 2: parent is at index 0
		...
		it('should return true when parent class includes "FromExpr"', () => {
			const parentNode = {
		...
		it('should return true when parent has types array', () => {
			const parentNode = {
				[nodeClassKey]: 'apex.jorje.data.ast.SomeNode',
				types: [],
		...
		it('should return false when parent class is undefined', () => {
		...
		it('should return false when stack is too short', () => {
			const stack: unknown[] = [];
			const path = createMockPath({} as ApexNode, undefined, stack);
			expect(isInTypeContext(path)).toBe(false);
		});
	...
	describe('createTypeNormalizingPrint', () => {
		it('should return original print when shouldNormalizeType is false', () => {
		...
		it('should normalize single identifier with standard object type', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(mockPrint);
			const node = {
				[nodeClassKey]: 'apex.jorje.data.ast.Identifier',
		...
		it('should handle empty string in identifier value', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(mockPrint);
			const node = {
				[nodeClassKey]: 'apex.jorje.data.ast.Identifier',
		...
		it('should handle node with names array when key is names', () => {
			const mockPrint = createMockPrint();
		...
		it('should handle node with names array where names is not an array', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(
				mockPrint,
				true,
		...
		it('should handle names array with nodes that have no value property', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(
				mockPrint,
				true,
		...
		it('should handle names array with nodes that have empty string value', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(
				mockPrint,
				true,
		...
		it('should handle names array with no changes (values already normalized)', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(
				mockPrint,
				true,
		...
		it('should handle names array restoration with node missing value property', () => {
			const mockPrint = createMockPrint();
		...
		it('should handle names array with non-object primitive values', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(
				mockPrint,
				true,
		...
		it('should handle key being undefined', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(mockPrint);
			const node = {
				[nodeClassKey]: 'apex.jorje.data.ast.MethodDecl',
		...
		it('should handle forceTypeContext parameter', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(
				mockPrint,
				true,
		...
		it('should handle parentKey parameter', () => {
			const mockPrint = createMockPrint();
			const typeNormalizingPrint = createTypeNormalizingPrint(
				mockPrint,
				false,
