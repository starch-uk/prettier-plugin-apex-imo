...
describe('index', () => {
	describe('plugin structure', () => {
		it('exports plugin object with correct type', () => {
		...
		it('exports apex printer with print method', () => {
	...
	describe('isApexParser', () => {
		...
		it('should return false for undefined parser', () => {
			expect(isApexParser(undefined)).toBe(false);
		});

		it('should return false for non-string parser', () => {
			expect(isApexParser(123 as unknown as string)).toBe(false);
		});
	});

	describe('shouldSkipCodeBlock', () => {
		it('should return true for empty formatted code', () => {
			const params: ShouldSkipCodeBlockParams = {
				endPos: 4,
				formattedCode: '   ',
				originalCode: 'test',
				processedText: 'text',
			...
			expect(shouldSkipCodeBlock(params)).toBe(true);
		});

		it('should return true for formatted code starting with FORMAT_FAILED_PREFIX', () => {
			const params: ShouldSkipCodeBlockParams = {
				endPos: 4,
		...
		it('should return true when formatted equals original and contains newlines', () => {
			const params: ShouldSkipCodeBlockParams = {
				endPos: 16,
				formattedCode: 'test\ncode',
				originalCode: 'test\ncode',
		...
		it('should return false when formatted equals original but no newlines', () => {
			const params: ShouldSkipCodeBlockParams = {
				endPos: 11,
				formattedCode: 'test',
				originalCode: 'test',
			...
			expect(shouldSkipCodeBlock(params)).toBe(false);
		});

		it('should return false when formatted differs from original', () => {
			const params: ShouldSkipCodeBlockParams = {
				endPos: 4,
	...
	describe('createPreprocess', () => {
		it('should return text unchanged when parser is not a string', async () => {
		...
		it('should return text unchanged when parser is not an apex parser', async () => {
			const preprocess = createPreprocess(plugin);
			const text = 'public class Test {}';
			const result = await preprocess(text, {
				parser: 'typescript',
		...
		it('should return text unchanged when no code blocks are found', async () => {
			const preprocess = createPreprocess(plugin);
			const text = '/**\n * Regular comment\n */\npublic class Test {}';
			const result = await preprocess(text, {
				parser: 'apex',
	...
	describe('wrapParsers', () => {
		it('should return null when parsers is null', () => {
		...
		it('should return undefined when parsers is undefined', () => {
			const result = wrapParsers(undefined, plugin);
			expect(result).toBeUndefined();
		});

		it('should skip parsers with undefined values', () => {
			const mockParsers = {
				apex: undefined,
		...
		it('should skip parsers without parse function', () => {
			const mockParsers = {
				apex: {
					// No parse function
				},
		...
		it('should handle parsers without preprocess function', async () => {
		...
		it('should skip parsers with inherited properties (not own properties)', () => {
		...
		it('should wrap parsers with both parse and preprocess functions', async () => {
			const mockParse = vi.fn();
	...
	describe('shouldSkipCodeBlock function', () => {
		it('should skip code blocks with empty formatted code', async () => {
		...
		it('should skip code blocks that start with FORMAT_FAILED_PREFIX', async () => {
		...
		it('should skip code blocks when formatted code equals original and contains newlines', async () => {
			const apexParser = plugin.parsers?.apex;
			if (!apexParser?.preprocess) {
				throw new Error('apex parser preprocess not found');
			}
